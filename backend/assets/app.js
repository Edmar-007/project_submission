...
document.querySelectorAll('[data-multistep]').forEach((form) => {
  ...
  form.querySelectorAll('[data-next]').forEach((btn) => {
    btn.addEventListener('click', (e) => {
      // Validate visible panel inputs before moving next
      const current = panels[index];
      const invalid = Array.from(current.querySelectorAll('input,select,textarea')).find((el) => !el.disabled && !el.checkValidity());
      if (invalid) {
        // show native validation message if available
        if (typeof invalid.reportValidity === 'function') {
          invalid.reportValidity();
        } else {
          window.alert(invalid.validationMessage || 'Please fill required fields.');
        }
...
const MODAL_FOCUSABLE = 'button:not([disabled]), [href]:not([aria-disabled="true"]), input:not([disabled]):not([type="hidden"]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"]):not([disabled])';
let modalStack = [];
...
const syncBodyModalLock = () => {
  body.classList.toggle('modal-open', modalStack.length > 0);
};

const getFocusableElements = (root) => Array.from(root.querySelectorAll(MODAL_FOCUSABLE))
  .filter((el) => el.offsetParent !== null || el === document.activeElement);

const focusModalStart = (modal) => {
  const preferred = modal.querySelector('[data-modal-focus]');
  if (preferred && !preferred.disabled) {
    preferred.focus({ preventScroll: true });
    return;
  }
  getFocusableElements(modal)[0]?.focus({ preventScroll: true });
};

const resetModalBodyScroll = (modal) => {
  modal.querySelectorAll('.modal-body').forEach((bodyNode) => {
    bodyNode.scrollTop = 0;
  });
};

const getModalFromElement = (element) => element?.closest?.('.modal-backdrop') || null;

const ensureModalStructure = (modal) => {
  if (!modal || modal.dataset.modalNormalized === '1') return modal;
  modal.classList.add('modal-backdrop');
  if (!modal.hasAttribute('aria-hidden')) modal.setAttribute('aria-hidden', 'true');
  if (modal.dataset.overlayClose == null) modal.dataset.overlayClose = '1';

  let overlay = Array.from(modal.children).find((child) => child.classList.contains('modal-overlay'));
  if (!overlay) {
  ...
  let container = Array.from(modal.children).find((child) => child.classList.contains('modal-container'));
  if (!container) {
    container = document.createElement('div');
    container.className = 'modal-container';
    const cardCandidate = Array.from(modal.children).find((child) => child.classList.contains('modal-card'));
    if (cardCandidate) {
      container.appendChild(cardCandidate);
    }
    modal.appendChild(container);
  }

  let card = Array.from(container.children).find((child) => child.classList.contains('modal-card'));
  if (!card) {
  ...
  const head = Array.from(card.children).find((child) => child.classList.contains('modal-head'));
  const bodyBlock = Array.from(card.children).find((child) => child.classList.contains('modal-body'));
  const footer = Array.from(card.children).find((child) => child.classList.contains('modal-footer'));

  if (!bodyBlock) {
    const body = document.createElement('div');
    body.className = 'modal-body';
    const toMove = Array.from(card.children).filter((child) => child !== head && child !== footer);
  ...
  modal.dataset.modalNormalized = '1';
  return modal;
};

const closeModal = (modal, { restoreFocus = true } = {}) => {
  if (!modal || modal.classList.contains('is-closing')) return;
  const normalized = ensureModalStructure(modal);
  const stackIndex = modalStack.lastIndexOf(normalized);
  if (stackIndex === -1 && !normalized.classList.contains('open')) return;

  normalized.classList.remove('open');
  normalized.classList.add('is-closing');
  normalized.setAttribute('aria-hidden', 'true');

  window.setTimeout(() => {
    normalized.classList.remove('is-closing');
    const idx = modalStack.lastIndexOf(normalized);
    ...
    const opener = modalOpeners.get(normalized);
...
const openModalByKey = (key, opener = null) => {
  ...
  const modal = document.querySelector(`[data-modal="${selectorKey}"]`);
  if (!modal) return null;

  const normalized = ensureModalStructure(modal);
  const currentTop = modalStack[modalStack.length - 1];
  if (currentTop === normalized) return normalized;

  const existingIndex = modalStack.lastIndexOf(normalized);
  if (existingIndex !== -1) modalStack.splice(existingIndex, 1);
  if (opener) modalOpeners.set(normalized, opener);

  normalized.classList.remove('is-closing');
  normalized.classList.add('open');
  normalized.setAttribute('aria-hidden', 'false');
  modalStack.push(normalized);
  syncBodyModalLock();
  resetModalBodyScroll(normalized);
  window.setTimeout(() => focusModalStart(normalized), 40);

  const chainTo = normalized.dataset.chainNextModal;
  if (chainTo) {
    window.setTimeout(() => openModalByKey(chainTo, opener), MODAL_ANIMATION_MS + 40);
  }
  return normalized;
};

document.querySelectorAll('.modal-backdrop').forEach(ensureModalStructure);

document.addEventListener('click', (event) => {
  const openTrigger = event.target.closest('[data-open-modal]');
  if (openTrigger) {
    event.preventDefault();
    const modalKey = openTrigger.getAttribute('data-open-modal');
    const sourceModal = getModalFromElement(openTrigger);
    if (openTrigger.hasAttribute('data-close-modal') && sourceModal) {
      closeModal(sourceModal, { restoreFocus: false });
    }
    openModalByKey(modalKey, openTrigger);
    return;
  }
  ...
  if (closeTrigger) {
    event.preventDefault();
    closeModal(getModalFromElement(closeTrigger));
    return;
  }

  const backdrop = event.target.closest('.modal-backdrop.open');
  if (!backdrop) return;
  const allowOverlayClose = backdrop.dataset.overlayClose !== '0';
  if (!allowOverlayClose) return;
  if (event.target === backdrop || event.target.classList.contains('modal-overlay')) {
    closeModal(backdrop);
  }
});
...
// Quick-create activity modal validation
document.querySelectorAll('.modal-backdrop form').forEach((form) => {
  const actionInput = form.querySelector('input[name="action"]');
  if (!actionInput || actionInput.value !== 'quick_create_activity') return;

  const validateQuickCreate = () => {
    const title = (form.querySelector('input[name="title"]')?.value || '').trim();
    const minMembers = parseInt(form.querySelector('input[name="min_members"]')?.value || '1', 10) || 1;
    const maxMembers = parseInt(form.querySelector('input[name="max_members"]')?.value || '1', 10) || 1;
    const maxRes = parseInt(form.querySelector('input[name="max_resubmissions"]')?.value || '1', 10) || 1;
    if (!title) {
      window.alert('Provide a title for the submission.');
      return false;
    }
    if (minMembers < 1) {
      window.alert('Min members must be at least 1.');
      return false;
    }
    if (maxMembers < minMembers) {
      window.alert('Max members cannot be less than min members.');
      return false;
    }
    if (maxRes < 1) {
      window.alert('Max resubmissions must be at least 1.');
      return false;
    }
    return true;
  };
...
window.modalManager = { openModalByKey, closeModal, ensureModalStructure };
...
const createGeneratedModal = ({ key, title, lead, contentNode, maxWidth = '' }) => {
  ...
  backdrop.innerHTML = `
    ...
    <div class="modal-container">
      <div class="modal-card${maxWidth ? ' modal-lg' : ''}" role="dialog" aria-modal="true">
        ...
        <div class="modal-body generated-modal-form"></div>
  ...
  const bodyWrap = backdrop.querySelector('.generated-modal-form');
  if (maxWidth && card) card.style.maxWidth = maxWidth;
  bodyWrap?.appendChild(contentNode);
  document.body.appendChild(backdrop);
  return ensureModalStructure(backdrop);
};
...
const ensureAjaxDetailModal = () => {
  ...
  modal.innerHTML = `
    ...
    <div class="modal-container">
      <div class="modal-card modal-card-xl ajax-detail-card" role="dialog" aria-modal="true" aria-labelledby="ajax-detail-title">
        ...
        <div class="modal-body ajax-detail-body" data-ajax-detail-body></div>
        <div class="modal-footer">
          <div class="modal-actions">
            <button type="button" class="btn btn-outline" data-close-modal>Close</button>
          </div>
  ...
  return ensureModalStructure(modal);
};
...
const openAjaxDetailModal = async (trigger) => {
  ...
  try {
    ...
    bodyHost.querySelectorAll('.modal-backdrop').forEach(ensureModalStructure);
  } catch (error) {
