...
<div class="app-shell">
<?php if ($role): ?>
  <aside class="sidebar <?= $role === 'student' ? 'student-sidebar-shell' : '' ?>">
    ...
    <nav class="side-nav <?= $role === 'student' ? 'student-side-nav-modern' : '' ?>">
      <?php foreach (($nav[$role] ?? []) as $label => $path): ?>
        ...
        <a class="<?= $isActive ? 'active' : '' ?>" href="<?= h(url($path)) ?>">
    ...
    <div class="sidebar-footer <?= $role === 'student' ? 'student-sidebar-profile' : '' ?>">
      <div class="student-sidebar-user">
        <div class="student-avatar student-avatar-small" data-student-avatar data-avatar-initial="<?= h($avatarInitial) ?>"><?= h($avatarInitial) ?></div>
        <div>
...
<main class="main-content <?= $role ? '' : 'no-sidebar' ?>">
  <header class="topbar modern-topbar <?= $role === 'student' ? 'student-topbar-shell' : '' ?>">
    <div class="topbar-heading-wrap">
      <?php if ($role): ?>
        <button type="button" class="icon-btn sidebar-toggle-btn" data-sidebar-toggle aria-label="Toggle navigation">
    ...
    <?php if ($role && $user): ?>
      <div class="topbar-meta modern-meta <?= $role === 'student' ? 'student-topbar-meta' : '' ?>">
        ...
        <div class="notification-menu">
          <button type="button" class="icon-btn" data-toggle-menu>
            <span>🔔</span>
            <?php if ($unreadCount > 0): ?><span class="badge-count"><?= (int) $unreadCount ?></span><?php endif; ?>
          </button>
          <div class="notification-dropdown" data-menu>
            <div class="dropdown-head">
              <strong>Notifications</strong>
              <a href="<?= h($notificationPage) ?>">View all</a>
            </div>
            <?php if ($recentNotifications): ?>
              <?php foreach ($recentNotifications as $note): ?>
                <div class="notification-item <?= (int) $note['is_read'] === 0 ? 'unread' : '' ?>">
        ...
        <div class="profile-menu dropdown-shell <?= $role === 'student' ? 'student-topbar-usercard' : '' ?>" data-dropdown-shell>
          <button type="button" class="profile-toggle" data-dropdown-toggle aria-expanded="false" aria-label="Open profile menu">
            <div class="profile-toggle-copy">
              <div class="pill soft"><?= h(ucfirst($role)) ?></div>
              <strong><?= h($user['username'] ?? $user['full_name'] ?? 'User') ?></strong>
            </div>
            <div class="student-avatar" data-student-avatar data-avatar-initial="<?= h($avatarInitial) ?>"><?= h($avatarInitial) ?></div>
          </button>
