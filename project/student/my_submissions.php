...
<section class="workspace-shell student-history-shell">
  ...
  <?php foreach ($rows as $row): ?>
    ...
    ?>
    <article class="card submission-history-record">
      <div class="split-header submission-history-summary">
        <div>
          <div class="eyebrow"><?= h($row['subject_code']) ?> · <?= h($row['activity_title'] ?: 'General submission') ?> · <?= h($row['team_name']) ?></div>
          <h3 class="section-title"><?= h($row['assigned_system'] ?: 'Untitled Project') ?></h3>
          <div class="muted small"><?= h($row['subject_name']) ?> · <?= h($row['activity_title'] ?: 'General submission') ?> · Leader: <?= h($row['leader_name']) ?> · Your role: <?= h(ucfirst((string) $row['member_role'])) ?></div>
        </div>
      ...
      <div class="history-feedback-panel">
        ...
        <div class="submission-primary-actions">
          ...
          <?php if (!empty($row['attachment_path'])): ?><a class="btn btn-ghost" target="_blank" href="<?= h(url($row['attachment_path'])) ?>">Attachment</a><?php endif; ?>
          <?php if ($canManage): ?>
            <a class="btn" href="<?= h(url('student/submission_edit.php?id=' . (int) $row['id'])) ?>">Edit latest version</a>
  ...
  <?php if (!$rows): ?>
    <div class="card empty-state" style="padding:32px;">No team submission is visible yet. Once a leader submits a project, this page will show the current record and its full version history.</div>
  <?php endif; ?>
</section>
