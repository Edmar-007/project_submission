...
function ensure_submission_activity_schema(): void {
    ...
    $tables = [
        ...
        'submissions' => ['activity_id' => "ALTER TABLE submissions ADD COLUMN activity_id INT NULL AFTER subject_id"],
        'submission_members' => [
    ...
    if (!isset($submissionIndexes['uniq_team_activity'])) {
        try { $pdo->exec('ALTER TABLE submissions ADD UNIQUE KEY uniq_team_activity (team_id, activity_id)'); } catch (Throwable $e) {}
    }
...
function student_subjects(int $sectionId): array {
    $stmt = pdo()->prepare('SELECT subj.*, t.full_name AS teacher_name, ss.submission_deadline AS section_submission_deadline, ss.deadline_warning_hours AS section_deadline_warning_hours FROM section_subjects ss JOIN subjects subj ON subj.id = ss.subject_id JOIN teachers t ON t.id = subj.teacher_id WHERE ss.section_id = ? AND subj.status = "active" ORDER BY subj.subject_name');
...
function team_member_rows(int $teamId): array {
    $stmt = pdo()->prepare('SELECT tm.role, st.id, st.student_id, st.full_name FROM team_members tm JOIN students st ON st.id = tm.student_id WHERE tm.team_id = ? ORDER BY CASE WHEN tm.role = "leader" THEN 0 ELSE 1 END, st.full_name ASC');
    $stmt->execute([$teamId]);
    return $stmt->fetchAll();
}
...
function student_visible_activities(int $studentId, int $sectionId, ?int $subjectId = null): array {
    ...
    $sql .= ' ORDER BY subj.subject_name ASC, COALESCE(act.deadline_at, act.created_at) ASC, act.title ASC';
    $stmt = pdo()->prepare($sql);
    $stmt->execute($params);
    $rows = [];
    $seen = [];
