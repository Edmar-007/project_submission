<?php
if (defined('FILE_TEACHER_NOTIFICATIONS_PHP_LOADED')) { return; }
define('FILE_TEACHER_NOTIFICATIONS_PHP_LOADED', true);

require_once __DIR__ . '/../backend/config/app.php';
require_role('teacher');
$teacher = current_user();
// If a single notification is clicked from the topbar, mark it as read and jump to it
if (isset($_GET['mark'])) {
    $markId = (int) ($_GET['mark'] ?? 0);
    if ($markId > 0) {
        mark_notification_read('teacher', (int) $teacher['id'], $markId);
        redirect_to('teacher/notifications.php#notification-' . $markId);
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'mark_read') {
        ...
        if ($noteId > 0) {
            mark_notification_read('teacher', (int) $teacher['id'], $noteId);
            set_flash('success', 'Notification marked as read.');
            redirect_to('teacher/notifications.php#notification-' . $noteId);
        }
    }
}
