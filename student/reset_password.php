<?php
if (defined('FILE_STUDENT_RESET_PASSWORD_PHP_LOADED')) { return; }
define('FILE_STUDENT_RESET_PASSWORD_PHP_LOADED', true);

require_once __DIR__ . '/../backend/config/app.php';
$token = trim($_GET['token'] ?? $_POST['token'] ?? '');
$stmt = pdo()->prepare('SELECT * FROM password_reset_tokens WHERE token = ? AND user_type = "student" AND used_at IS NULL AND expires_at > NOW() LIMIT 1');
$stmt->execute([$token]);
$record = $stmt->fetch();
if (!$record) {
