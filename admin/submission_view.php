...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    create_notification('student', (int) $submission['student_id'], 'Submission updated', 'Your submission for ' . $submission['subject_name'] . ' is now ' . $status . ($grade ? ' with grade ' . $grade : '') . '.', $status === 'graded' ? 'success' : 'info');
...
<div class="detail-grid">
  <div class="detail-section">
    <div class="card">
      ...
      <div class="form-actions" style="margin-top:16px;"><a class="btn btn-secondary" target="_blank" href="<?= h($submission['project_url']) ?>">Open project</a><a class="btn btn-secondary" target="_blank" href="<?= h($submission['video_url']) ?>">Open video</a><a class="btn btn-outline" href="<?= h(url('admin/student_view.php?id=' . (int) $submission['student_id'])) ?>">Open student</a></div>
    </div>
    <div class="card">
    ...
    <div class="card">
      ...
      <div class="grid cols-2">
        <div class="callout"><strong>Demo login</strong><div class="muted small">Username: <?= h($submission['admin_username'] ?: '—') ?></div><div class="muted small">Password: <?= h($submission['admin_password'] ?: '—') ?></div></div>
        <div class="callout"><strong>Legacy extra login</strong><div class="muted small">Username: <?= h($submission['user_username'] ?: '—') ?></div><div class="muted small">Password: <?= h($submission['user_password'] ?: '—') ?></div></div>
      </div>
