    ...
    <table class="table table-hover align-middle">
      ...
      <?php foreach ($rows as $row): ?>
        <tr>
          ...
          <td><strong><?= h($row['assigned_system']) ?></strong><div class="muted small"><?= h($row['company_name'] ?: 'No company label') ?></div><div class="mt-2 d-flex gap-2 flex-wrap"><?php if (!empty($row['project_url'])): ?><a class="btn btn-outline btn-sm" href="<?= h($row['project_url']) ?>" target="_blank" rel="noopener"><i class="bi bi-box-arrow-up-right"></i> Project</a><?php endif; ?><?php if (!empty($row['video_url'])): ?><a class="btn btn-outline btn-sm" href="<?= h($row['video_url']) ?>" target="_blank" rel="noopener"><i class="bi bi-play-btn"></i> Video</a><?php endif; ?></div></td>
          ...
          <td class="text-end">
            <div class="icon-action-group justify-content-end">
              <a class="icon-action" href="<?= h(url('admin/submission_view.php?id=' . (int) $row['id'])) ?>" title="Open submission"><i class="bi bi-eye"></i></a>
...
<div class="modal-backdrop" data-modal="submission-guide" aria-hidden="true"><div class="modal-card" role="dialog" aria-modal="true"><div class="modal-head"><div><span class="badge-soft"><i class="bi bi-info-circle"></i> Submission Guide</span><h3>How to use this registry</h3></div><button type="button" class="icon-btn modal-close" data-close-modal aria-label="Close">✕</button></div><div class="stack"><div class="callout"><strong>Filter by accountability</strong><div class="muted small">Use subject, section, and status before opening quick review.</div></div><div class="callout"><strong>Archive historical records only</strong><div class="muted small">Keep active submissions visible until grading is completed.</div></div><div class="callout"><strong>Use quick review for small changes</strong><div class="muted small">Open the full page only when you need the richer submission context.</div></div></div></div></div>
<?php foreach ($rows as $row): ?>
<div class="modal-backdrop" data-modal="review-<?= (int) $row['id'] ?>" aria-hidden="true">
  <div class="modal-card modal-lg" role="dialog" aria-modal="true">
    ...
    <form method="post" class="form-modal-grid">
      ...
      <div class="full d-flex justify-content-between gap-2 flex-wrap"><button class="btn btn-outline" type="button" data-close-modal>Cancel</button><div class="d-flex gap-2"><button class="btn btn-danger" type="submit" name="action" value="archive" data-confirm-title="Archive submission?" data-confirm-message="This submission will be removed from the active registry."><i class="bi bi-archive"></i> Archive</button><button class="btn" type="submit"><i class="bi bi-save"></i> Save Review</button></div></div>
    </form>
  </div>
</div>
<?php endforeach; ?>
