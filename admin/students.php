<?php
if (defined('FILE_ADMIN_STUDENTS_PHP_LOADED')) { return; }
define('FILE_ADMIN_STUDENTS_PHP_LOADED', true);
...
<div class="modal-backdrop" data-modal="student-edit-<?= (int) $row['id'] ?>" aria-hidden="true">
  ...
  <div class="modal-card modal-lg" role="dialog" aria-modal="true">
    ...
    <form id="student-edit-form-<?= (int) $row['id'] ?>" method="post" class="form-modal-grid modal-body">
      <input type="hidden" name="_csrf" value="<?= h(csrf_token()) ?>">
    ...
    <div class="modal-footer">
      <div class="modal-actions">
        <?php if ($row['account_status'] !== 'archived'): ?>
        <button class="btn btn-outline" name="action" value="archive" type="submit" form="student-edit-form-<?= (int) $row['id'] ?>" data-confirm-title="archive student?" data-confirm-message="This student will move to archived status and become read only.">archive</button>
        <?php endif; ?>
        <button class="btn btn-outline" type="button" data-close-modal>close</button>
        <button class="btn" type="submit" form="student-edit-form-<?= (int) $row['id'] ?>">save</button>
      </div>
    </div>
  </div>
  </div>
