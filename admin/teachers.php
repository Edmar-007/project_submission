<?php
if (defined('FILE_ADMIN_TEACHERS_PHP_LOADED')) { return; }
define('FILE_ADMIN_TEACHERS_PHP_LOADED', true);
...
<div class="modal-backdrop" data-modal="admin-add-teacher" aria-hidden="true">
  <div class="modal-overlay"></div>
  <div class="modal-container">
    <div class="modal-card" role="dialog" aria-modal="true">
      <div class="modal-head">
        <div><span class="badge-soft"><i class="bi bi-person-plus"></i> Teacher</span><h3>Add Teacher</h3></div>
        <button type="button" class="icon-btn modal-close" data-close-modal aria-label="Close">✕</button>
      </div>
      <form id="admin-add-teacher-form" method="post" class="form-modal-grid modal-body">
        <input type="hidden" name="_csrf" value="<?= h(csrf_token()) ?>">
        <input type="hidden" name="action" value="create">
        <div><label>Teacher ID</label><input class="form-control" name="teacher_id" required></div>
        <div><label>Full name</label><input class="form-control" name="full_name" required></div>
        <div><label>Email</label><input class="form-control" type="email" name="email" required></div>
        <div><label>Username</label><input class="form-control" name="username" required></div>
        <div class="full"><label>Password</label><input class="form-control" type="password" name="password" required></div>
      </form>
      <div class="modal-footer">
        <div class="modal-actions">
          <button class="btn btn-outline" type="button" data-close-modal>Cancel</button>
          <button class="btn" type="submit" form="admin-add-teacher-form">Create teacher</button>
        </div>
      </div>
    </div>
  </div>
...
<div class="modal-backdrop" data-modal="edit-teacher-<?= (int) $row['id'] ?>" aria-hidden="true">
  <div class="modal-overlay"></div>
  <div class="modal-container">
    <div class="modal-card" role="dialog" aria-modal="true">
      <div class="modal-head">
        <div><span class="badge-soft"><i class="bi bi-pencil"></i> Teacher</span><h3>Edit <?= h($row['full_name']) ?></h3></div>
        <button type="button" class="icon-btn modal-close" data-close-modal aria-label="Close">✕</button>
      </div>
      <form id="edit-teacher-form-<?= (int) $row['id'] ?>" method="post" class="form-modal-grid modal-body">
        <input type="hidden" name="_csrf" value="<?= h(csrf_token()) ?>">
        <input type="hidden" name="action" value="save">
        <input type="hidden" name="id" value="<?= (int) $row['id'] ?>">
        <div><label>Full name</label><input class="form-control" name="full_name" value="<?= h($row['full_name']) ?>" required></div>
        <div><label>Email</label><input class="form-control" type="email" name="email" value="<?= h($row['email']) ?>" required></div>
        <div><label>Username</label><input class="form-control" name="username" value="<?= h($row['username']) ?>" required></div>
        <div><label>Status</label><select class="form-select" name="status"><option value="active" <?= selected($row['status'],'active') ?>>Active</option><option value="inactive" <?= selected($row['status'],'inactive') ?>>Inactive</option></select></div>
        <div class="full"><label>Reset password</label><input class="form-control" type="password" name="password" placeholder="Leave blank to keep current password"></div>
      </form>
      <div class="modal-footer">
        <div class="modal-actions">
          <button class="btn btn-outline" type="button" data-close-modal>Cancel</button>
          <button class="btn" type="submit" form="edit-teacher-form-<?= (int) $row['id'] ?>">Save changes</button>
        </div>
      </div>
    </div>
  </div>
