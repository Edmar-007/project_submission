...
<section class="table-card table-bootstrap-shell">
  ...
  <div class="table-responsive"><table class="table table-hover align-middle"><thead><tr><th>Teacher</th><th>Coverage</th><th>Status</th><th class="text-end">Actions</th></tr></thead><tbody><?php foreach ($teachers as $row): ?><tr><td><strong><?= h($row['full_name']) ?></strong><div class="muted small"><?= h($row['teacher_id']) ?> · <?= h($row['username']) ?></div><div class="muted small"><?= h($row['email']) ?></div></td><td><div class="d-flex gap-2 flex-wrap"><span class="badge-soft"><i class="bi bi-book"></i> <?= (int) $row['total_subjects'] ?> subjects</span><span class="badge-soft"><i class="bi bi-diagram-3"></i> <?= (int) $row['total_sections'] ?> sections</span></div></td><td><?= status_badge($row['status']) ?></td><td class="text-end"><div class="icon-action-group justify-content-end"><a class="icon-action" href="<?= h(url('admin/teacher_view.php?id=' . (int) $row['id'])) ?>" title="View teacher"><i class="bi bi-eye"></i></a><button class="icon-action" type="button" data-open-modal="edit-teacher-<?= (int) $row['id'] ?>" title="Edit teacher"><i class="bi bi-pencil"></i></button></div></td></tr><?php endforeach; ?><?php if (!$teachers): ?><tr><td colspan="4" class="table-empty">No teacher records matched your filters.</td></tr><?php endif; ?></tbody></table></div>
</section>
