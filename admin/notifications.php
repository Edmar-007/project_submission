...
if (isset($_GET['mark']) && $_GET['mark'] === 'all') {
    ...
    set_flash('success', 'All admin notifications marked as read.');
    redirect_to('admin/notifications.php');
}
if (isset($_GET['note'])) {
    mark_notification_read('admin', (int) $admin['id'], (int) $_GET['note']);
...
<div class="split-header">
  ...
  <a class="btn btn-secondary" href="<?= h(url('admin/notifications.php?mark=all')) ?>">Mark all as read</a>
</div>
    ...
    <div class="timeline-item <?= (int) $row['is_read'] === 0 ? 'unread' : '' ?>">
      <div class="notification-title-row">
        ...
        <div class="table-actions">
          ...
          <?php if ((int) $row['is_read'] === 0): ?><a class="btn btn-outline" href="<?= h(url('admin/notifications.php?note=' . (int) $row['id'])) ?>">Mark read</a><?php endif; ?>
        </div>
      </div>
      <p><?= h($row['message']) ?></p>
