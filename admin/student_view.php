<?php
if (defined('FILE_ADMIN_STUDENT_VIEW_PHP_LOADED')) { return; }
define('FILE_ADMIN_STUDENT_VIEW_PHP_LOADED', true);
...
<div class="detail-grid">
  <div class="detail-section">
    ...
    <div class="card">
      ...
      <div class="table-wrap"><table class="table-redesign"><thead><tr><th>System</th><th>Status</th><th>Grade</th><th>Date</th><th></th></tr></thead><tbody><?php foreach ($submissions as $submission): ?><tr><td><?= h($submission['assigned_system']) ?></td><td><?= status_badge($submission['status']) ?></td><td><?= h($submission['grade'] ?: '—') ?></td><td><?= h($submission['submitted_at']) ?></td><td class="text-end"><a class="icon-action" href="<?= h(url('admin/submission_preview.php?id=' . (int) $submission['id'])) ?>" data-ajax-modal="1" data-modal-title="Submission overview" title="open submission" aria-label="open submission"><i class="bi bi-eye"></i></a></td></tr><?php endforeach; ?><?php if (!$submissions): ?><tr><td colspan="5" class="empty-state">No submissions for this student yet.</td></tr><?php endif; ?></tbody></table></div>
    </div>
  </div>
  <div class="detail-section">
    <div class="card">
