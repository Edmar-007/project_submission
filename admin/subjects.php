<?php
if (defined('FILE_ADMIN_SUBJECTS_PHP_LOADED')) { return; }
define('FILE_ADMIN_SUBJECTS_PHP_LOADED', true);
...
<div class="modal-backdrop" data-modal="admin-add-subject" aria-hidden="true">
  <div class="modal-overlay"></div>
  <div class="modal-container">
    <div class="modal-card modal-lg" role="dialog" aria-modal="true">
      <div class="modal-head">
        <div><span class="badge-soft"><i class="bi bi-book"></i> Subject</span><h3>Add Subject</h3></div>
        <button type="button" class="icon-btn modal-close" data-close-modal aria-label="Close">✕</button>
      </div>
      <form id="admin-add-subject-form" method="post" class="form-modal-grid modal-body">
        <input type="hidden" name="_csrf" value="<?= h(csrf_token()) ?>">
        <input type="hidden" name="action" value="create">
        <div><label>Subject code</label><input class="form-control" name="subject_code" required></div>
        <div><label>Subject name</label><input class="form-control" name="subject_name" required></div>
        <div><label>Teacher</label><select class="form-select" name="teacher_id" required><?php foreach ($teachers as $teacher): ?><option value="<?= (int) $teacher['id'] ?>"><?= h($teacher['full_name']) ?></option><?php endforeach; ?></select></div>
        <div><label>Deadline warning hours</label><input class="form-control" type="number" min="1" name="deadline_warning_hours" value="72"></div>
        <div><label>Submission deadline</label><input class="form-control" type="datetime-local" name="submission_deadline"></div>
        <div class="full"><label>Description</label><textarea class="form-control" name="description" rows="3"></textarea></div>
        <div class="full"><label>Assign sections</label><div class="row g-2"><?php foreach ($sections as $section): ?><div class="col-12 col-md-6"><label class="form-check"><input class="form-check-input" type="checkbox" name="section_ids[]" value="<?= (int) $section['id'] ?>"> <span class="form-check-label"><?= h($section['section_name']) ?></span></label></div><?php endforeach; ?></div></div>
      </form>
      <div class="modal-footer">
        <div class="modal-actions">
          <button class="btn btn-outline" type="button" data-close-modal>Cancel</button>
          <button class="btn" type="submit" form="admin-add-subject-form">Create subject</button>
        </div>
      </div>
    </div>
  </div>
...
<div class="modal-backdrop" data-modal="edit-subject-<?= (int) $row['id'] ?>" aria-hidden="true">
  <div class="modal-overlay"></div>
  <div class="modal-container">
    <div class="modal-card modal-lg" role="dialog" aria-modal="true">
      <div class="modal-head">
        <div><span class="badge-soft"><i class="bi bi-pencil"></i> Subject</span><h3>Edit <?= h($row['subject_name']) ?></h3></div>
        <button type="button" class="icon-btn modal-close" data-close-modal aria-label="Close">✕</button>
      </div>
      <form id="edit-subject-form-<?= (int) $row['id'] ?>" method="post" class="form-modal-grid modal-body">
        <input type="hidden" name="_csrf" value="<?= h(csrf_token()) ?>">
        <input type="hidden" name="action" value="save">
        <input type="hidden" name="subject_id" value="<?= (int) $row['id'] ?>">
        <div><label>Subject code</label><input class="form-control" name="subject_code" value="<?= h($row['subject_code']) ?>" required></div>
        <div><label>Subject name</label><input class="form-control" name="subject_name" value="<?= h($row['subject_name']) ?>" required></div>
        <div><label>Teacher</label><select class="form-select" name="teacher_id" required><?php foreach ($teachers as $teacher): ?><option value="<?= (int) $teacher['id'] ?>" <?= (int)$teacher['id']===(int)$row['teacher_id'] ? 'selected' : '' ?>><?= h($teacher['full_name']) ?></option><?php endforeach; ?></select></div>
        <div><label>Status</label><select class="form-select" name="status"><option value="active" <?= selected($row['status'],'active') ?>>Active</option><option value="inactive" <?= selected($row['status'],'inactive') ?>>Inactive</option><option value="archived" <?= selected($row['status'],'archived') ?>>Archived</option></select></div>
        <div><label>Submission deadline</label><input class="form-control" type="datetime-local" name="submission_deadline" value="<?= h(!empty($row['submission_deadline']) ? date('Y-m-d\TH:i', strtotime((string) $row['submission_deadline'])) : '') ?>"></div>
        <div><label>Deadline warning hours</label><input class="form-control" type="number" min="1" name="deadline_warning_hours" value="<?= (int)($row['deadline_warning_hours'] ?? 72) ?>"></div>
        <div class="full"><label>Description</label><textarea class="form-control" name="description" rows="3"><?= h($row['description']) ?></textarea></div>
        <div class="full"><label>Assign sections</label><div class="row g-2"><?php foreach ($sections as $section): ?><div class="col-12 col-md-6"><label class="form-check"><input class="form-check-input" type="checkbox" name="section_ids[]" value="<?= (int) $section['id'] ?>" <?= in_array((int) $section['id'], $assignedMap[(int)$row['id']] ?? [], true) ? 'checked' : '' ?>> <span class="form-check-label"><?= h($section['section_name']) ?></span></label></div><?php endforeach; ?></div></div>
      </form>
      <div class="modal-footer">
        <div class="modal-actions">
          <button class="btn btn-outline" type="button" data-close-modal>Cancel</button>
          <button class="btn" type="submit" form="edit-subject-form-<?= (int) $row['id'] ?>">Save changes</button>
        </div>
      </div>
    </div>
  </div>
